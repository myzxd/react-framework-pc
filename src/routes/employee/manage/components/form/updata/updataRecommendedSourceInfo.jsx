...
class UpdataRecommendedSourceInfo extends Component {
  static propTypes = {
  ...
  renderForm = () => {
    ...
    if (flag) {
      ...
      switch (initApplicationChannel) {
        case AccountApplyWay.company:
          initApplicationCompanyName = this.props.employeeDetail.application_company_name;
          break;
        case AccountApplyWay.recommend:
          initIdentityNo = this.props.employeeDetail.referrer_identity_no;
          initName = this.props.employeeDetail.referrer_name;
          initPhone = this.props.employeeDetail.referrer_phone;
          break;
        case AccountApplyWay.apply:
          initApplicationPlatform = this.props.employeeDetail.application_platform;
          break;
        case AccountApplyWay.transfer:
          transferSignPlatform = this.props.employeeDetail.transfer_sign_platform;
          transferSignSupplier = this.props.employeeDetail.transfer_sign_supplier;
          transferSignPost = this.props.employeeDetail.transfer_sign_post;
          break;
        default:
          break;
      }
      const formItems = [
        {
          ...
          form: getFieldDecorator('applicationChannelId', {
            ...
            <Select placeholder="请选择应聘渠道" allowClear onChange={this.onChangeRecruitmentChannel}>
      ...
      if (`${applicationChannelId}` === `${AccountApplyWay.company}`) {
        formItems.push({
          label: '公司名称',
      ...
      if (`${applicationChannelId}` === `${AccountApplyWay.recommend}`) {
        formItems.push({
          label: '推荐人身份证',
      ...
      // 当招聘渠道时，显示推招聘渠道表单
      if (`${applicationChannelId}` === `${AccountApplyWay.apply}`) {
        formItems.push({
          label: '渠道',
      ...
      // 当转签时，显示平台、供应商、岗位表单
      if (`${applicationChannelId}` === `${AccountApplyWay.transfer}`) {
        formItems.push(
          {
            label: '平台',
    ...
    const formItems = [
      {
        ...
        form: getFieldDecorator('recruitmentChannel', {
          rules: [{ required: true, message: '请选择内容' }],
          initialValue: initRecruitmentChannel ? `${initRecruitmentChannel}` : undefined,
        })(
        ...
        form: getFieldDecorator('referrerCompany',
          {
            ...
            <CommonSelectRecommendCompany
              ...
              optionFilterProp="children"
              placeholder="请选择推荐公司"
              suppliers={this.props.flag ? [] : supplierIds}
            />,
        ),
      });
    }
    ...
    if (`${recruitmentChannel}` === `${AccountRecruitmentChannel.recommend}`) {
      formItems.push({
        ...
        label: '推荐人姓名',
        key: 'staffIdentityName',
        form: getFieldDecorator('staffIdentityName', { initialValue: initdefineStaffIdentityName })(
          <Input disabled />,
        ),
      }, {
        label: '推荐人手机号',
...
export default connect(mapStateToProps)(UpdataRecommendedSourceInfo);
