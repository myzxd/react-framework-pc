...
function ExamineFlow(props) {
  const { dispatch, name,
    state,
    examineFlowList = {}, initItem = {},
    applyApplicationType, bizType, merchant } = props;
  const data = dot.get(examineFlowList, 'datas', []);
  const interfaceRef = useRef(true);
  useEffect(() => {
    // 防止重复调用接口
    if (name && interfaceRef.current === true) {
      dispatch({
        type: 'relationExamineFlow/fetchExamineFlow',
        payload: {
          flowNameKey: name,
          bizType,
          merchant,
  ...
  useEffect(() => {
    ...
    if (applyApplicationType) {
      dispatch({
        type: 'relationExamineFlow/reduceExamineFlow',
        payload: {} });
    }
    return () => {
      dispatch({
...
const mapStateToProps = ({ relationExamineFlow: { examineFlowList } }) => {
  return { examineFlowList };
};

export default connect(mapStateToProps)(ExamineFlow);
