...
function RelationExamineFlowAffair(props) {
  ...
  const renderTableOperate = (record) => {
    const operates = [];
    if (Operate.canOperateExpenseRelationExamineFlowAffairDetail()) {
      operates.push(
        <a
          key="detail"
          href={`/#/Expense/RelationExamineFlow/AffairDetail?qhFlowId=${dot.get(record, 'qh_flow_info._id', undefined)}&xdFlowId=${dot.get(record, 'xd_flow_info._id', undefined)}`}
          target="_blank"
    ...
    && Operate.canOperateExpenseRelationExamineFlowAffairUpdateState()) {
      operates.push(
    ...
    if (record.state === RelationExamineFlowAvailableState.noAvailable) {
      if (Operate.canOperateExpenseRelationExamineFlowAffairUpdate()) {
        operates.push(
          <a
            rel="noopener noreferrer"
            target="_blank"
            href={`/#/Expense/RelationExamineFlow/AffairUpdate?pluginId=${record.plugin_id}&appWatchFlowId=${record.app_watch_flow_id}&qhFlowId=${dot.get(record, 'qh_flow_info._id', undefined)}&xdFlowId=${dot.get(record, 'xd_flow_info._id', undefined)}`}
          >编辑</a>,
        );
      }
      if (Operate.canOperateExpenseRelationExamineFlowAffairUpdateState()) {
        operates.push(
  ...
  const renderContent = () => {
    ...
    // 事务适用类型
    const affairs = enumeratedValue[enumeratedType] || [];
    const columns = [
      {
        title: '序号',
        dataIndex: 'id',
      ...
      },
      {
        ...
        width: 120,
        render: (text) => {
          if (!text || text.length === 0 || affairs.length === 0) {
            return '--';
          }
          const nameList = text.map((item) => {
            const type = affairs.find(v => v.value === item) || {};
            return type.name;
          });
    ...
    let titleExt;
    // 判断权限
    if (Operate.canOperateExpenseRelationExamineFlowAffairCreate()) {
      titleExt = (<Button
        onClick={() => {
          window.location.href = '/#/Expense/RelationExamineFlow/AffairCreate';
        }}
        type="primary"
      >新增</Button>);
    }
    return (
      <CoreContent title="事务类审批流列表" titleExt={titleExt}>
        <Table
...
function mapStateToProps({
  relationExamineFlow: { relationExamineFlowList },
  applicationCommon: { enumeratedValue },
}) {
  return { relationExamineFlowList, enumeratedValue };
}
export default connect(mapStateToProps)(RelationExamineFlowAffair);
