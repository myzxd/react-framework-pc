...
function RelationExamineFlowCodeTeam(props) {
  ...
  const renderTableOperate = (record) => {
    const operates = [];
    if (Operate.canOperateExpenseRelationExamineFlowCodeTeamDetail()) {
      operates.push(
        <a
          key="detail"
          href={`/#/Expense/RelationExamineFlow/CodeTeamDetail?qhFlowId=${dot.get(record, 'qh_flow_info._id', undefined)}&xdFlowId=${dot.get(record, 'xd_flow_info._id', undefined)}`}
          target="_blank"
    ...
    && Operate.canOperateExpenseRelationExamineFlowCodeTeamUpdateState()) {
      operates.push(
    ...
    if (record.state === RelationExamineFlowAvailableState.noAvailable) {
      if (Operate.canOperateExpenseRelationExamineFlowCodeTeamUpdate()) {
        operates.push(
          <a
            rel="noopener noreferrer"
            target="_blank"
            href={`/#/Expense/RelationExamineFlow/CodeTeamUpdate?pluginId=${record.plugin_id}&appWatchFlowId=${record.app_watch_flow_id}&qhFlowId=${dot.get(record, 'qh_flow_info._id', undefined)}&xdFlowId=${dot.get(record, 'xd_flow_info._id', undefined)}`}
          >编辑</a>,
        );
      }
      if (Operate.canOperateExpenseRelationExamineFlowCodeTeamUpdateState()) {
        operates.push(
  ...
  const renderContent = () => {
      ...
      },
      {
        ...
        render: (text) => {
          if (Array.isArray(text)) {
            return text.map(item => CodeCostCenterType.description(item)).join('、');
          }
          return '--';
        },
      },
    ...
    let titleExt;
    // 判断权限
    if (Operate.canOperateExpenseRelationExamineFlowCodeTeamCreate()) {
      titleExt = (<Button
        onClick={() => {
          window.location.href = '/#/Expense/RelationExamineFlow/CodeTeamCreate';
        }}
        type="primary"
      >新增</Button>);
    }
    return (
      <CoreContent title="code/team审批流列表" titleExt={titleExt}>
        <Table
...
function mapStateToProps({ relationExamineFlow: { relationExamineFlowList } }) {
  return { relationExamineFlowList };
}
export default connect(mapStateToProps)(RelationExamineFlowCodeTeam);
