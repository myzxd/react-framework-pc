...
function RelationExamineFlowCost(props) {
  ...
  useEffect(() => {
    dispatch({
      ...
      payload: { enumeratedType, examineFlowBiz: ExpenseCostOrderBizType.costOf },
    });
  ...
  const renderTableOperate = (record) => {
    const operates = [];
    if (Operate.canOperateExpenseRelationExamineFlowCostDetail()) {
      operates.push(
        <a
          key="detail"
          href={`/#/Expense/RelationExamineFlow/CostDetail?qhFlowId=${dot.get(record, 'qh_flow_info._id', undefined)}&xdFlowId=${dot.get(record, 'xd_flow_info._id', undefined)}`}
          target="_blank"
    ...
    && Operate.canOperateExpenseRelationExamineFlowCostUpdateState()) {
      operates.push(
    ...
    if (record.state === RelationExamineFlowAvailableState.noAvailable) {
      if (Operate.canOperateExpenseRelationExamineFlowCostUpdate()) {
        operates.push(
          <a
            rel="noopener noreferrer"
            target="_blank"
            href={`/#/Expense/RelationExamineFlow/CostUpdate?pluginId=${record.plugin_id}&appWatchFlowId=${record.app_watch_flow_id}&qhFlowId=${dot.get(record, 'qh_flow_info._id', undefined)}&xdFlowId=${dot.get(record, 'xd_flow_info._id', undefined)}`}
          >编辑</a>,
        );
      }
      if (Operate.canOperateExpenseRelationExamineFlowCostUpdateState()) {
        operates.push(
  ...
  const renderContent = () => {
    ...
    let titleExt;
    // 判断权限
    if (Operate.canOperateExpenseRelationExamineFlowCostCreate()) {
      titleExt = (<Button
        onClick={() => {
          window.location.href = '/#/Expense/RelationExamineFlow/CostCreate';
        }}
        type="primary"
      >新增</Button>);
    }
    return (
      <CoreContent title="成本类审批流列表" titleExt={titleExt}>
        <Table
...
export default connect(mapStateToProps)(RelationExamineFlowCost);
