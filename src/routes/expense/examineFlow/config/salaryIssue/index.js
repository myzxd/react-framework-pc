/**
 * 审批流设置 - 审批流配置 - 服务费发放 /Expense/ExamineFlow/Config
 */
...
class SalaryIssueComponent extends Component {
  static getDerivedStateFromProps(prevProps, oriState) {
    const { examineFlowConfig = undefined, submitConfig = undefined } = oriState;
    const { salaryDistribute = {} } = prevProps;
    if (examineFlowConfig === undefined && Object.keys(salaryDistribute).length > 0) {
      return { examineFlowConfig: _.cloneDeep(salaryDistribute) };
    }

    if (submitConfig === undefined && Object.keys(salaryDistribute).length > 0) {
      return { submitConfig: _.cloneDeep(salaryDistribute) };
    }
    return null;
  }
  ...
  constructor() {
    ...
    this.state = {
      platformCodes,                                                         // 标签key
      examineFlowConfig: undefined,         // 审批流(服务费发放)配置数据
  ...
  componentDidMount = () => {
    ...
    // 获取审批流(服务费发放)配置数据
    this.props.dispatch({ type: 'expenseExamineFlow/fetchSalaryDistribute', payload: params });
  }

  componentWillUnmount() {
    this.props.dispatch({ type: 'expenseExamineFlow/resetSalaryDistribute' });
  }
  ...
  onChangeItems = (key, e) => {
    ...
    const value = {
      ...
      cost_group_id: e.expenseType,         // 费用分组id
      cost_accounting_id: e.subject,        // 科目id
      supplier_ids: e.supplier,             // 供应商id
      city_codes: e.cityCodes,              // 城市city_code
      city_spelling_list: e.city,           // 城市city_spelling
      ...
      subjectName: e.subjectName,           // 费用分组name
      typeName: e.typeName,                 // 科目name
    };
  ...
  onCreate = (key) => {
    ...
    if (is.not.existy(verifyData.flow_id)
    || is.empty(verifyData.supplier_ids)
    || is.empty(verifyData.city_codes)
    || is.empty(verifyData.biz_district_ids)
    || is.not.existy(verifyData.cost_accounting_id)
    || is.not.existy(verifyData.cost_group_id)) {
  ...
  onCopy = (value) => {
    ...
    if (examineFlowConfig[platformCodes].length !== 1
      && (is.not.existy(lastData.flow_id)
      || is.empty(lastData.supplier_ids)
      || is.empty(lastData.city_codes)
      || is.empty(lastData.biz_district_ids)
      || is.not.existy(lastData.cost_accounting_id)
      || is.not.existy(lastData.cost_group_id))) {
      examineFlowConfig[platformCodes].pop();
    }

    // 校验当前被拷贝的配置项，如果他都有值，才能进行拷贝操作
    if (is.not.existy(value.examineFlow) || is.empty(value.supplier) || is.empty(value.cityCodes) || is.empty(value.district) || is.not.existy(value.subject) || is.not.existy(value.expenseType)) {
      return message.error('您的配置项还没填写完成');
    }
    // 定义新组数数据为原数组数据，避免引用关系
    const copyData = {
      flow_id: value.examineFlow,                             // 审批流
      cost_group_id: value.subject,                 // 费用分组
      cost_accounting_id: value.expenseType,       // 科目
      supplier_ids: value.supplier,                   // 供应商
      city_codes: value.cityCodes,                       // 城市
      biz_district_ids: value.district,           // 商圈
  ...
  onVerify = (data) => {
    ...
    data.forEach((item) => {
      ...
      // 判断配置项中的科目是否选中
      if (!is.existy(item.cost_accounting_id)) {
        isPass = false;
      }
      // 判断配置项中的费用分组是否选中
      if (!is.existy(item.cost_group_id)) {
        isPass = false;
      }
      // 判断配置项中的供应商是否选中
  ...
  onSubmit = () => {
    ...
    // 提交时，只提交校验当前平台的编辑数据，其他平台数据不变，直接返回给后端
    const options = {};
    ...
    if (!this.onVerify(options[platformCodes])) {
      return message.error('您的审批流配置（服务费发放配置）还有未选中项!');
    }
    // 提交后台
    this.props.dispatch({ type: 'expenseExamineFlow/updateSalaryDistribute',
      payload: {
  ...
  renderContent = () => {
    ...
    return (
      <div className={style['app-comp-expense-salary-issue-component-content']}>
        {
          dataSource.map((item, index, records) => {
            ...
            const uniqueKey = item.uniqueKey ? item.uniqueKey : `salaryIssue${platformCodes}${item.flow_id}${index}`;
            ...
            const itemProps = {
              ...
              value: {
                ...
                expenseType: item.cost_group_id,
                subject: item.cost_accounting_id,
  ...
  render() {
    return (
      <div>
        ...
        {/* 渲染底部操作按钮 */}
        <div className={style['app-comp-expense-salary-issue-component-button']}>
          <Button type="primary" onClick={this.onSubmit} className={style['app-comp-expense-salary-issue-component-submit']}>确定</Button>
...
function mapStateToProps({ expenseExamineFlow: { salaryDistribute } }) {
  return { salaryDistribute };
}
export default connect(mapStateToProps)(SalaryIssueComponent);
