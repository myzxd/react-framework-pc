...
class PaymentInfo extends Component {
  ...
  renderPaymentInfo = () => {
    ...
    const contractDate = rentCycleStartAt && rentCycleEndAt
      ...
      : '--';
    // 续签合同租金期间
    const renewDate = contractStartDate && allocationEndDate ? `${moment(`${contractStartDate}`).format('YYYY.MM.DD')}-${moment(`${allocationEndDate}`).format('YYYY.MM.DD')}` : '--';
    ...
    return (
      <CoreContent title="最近一次付款信息">
        {
          rentCycleStartAt && rentCycleEndAt
            ?
            (
              <div>
                <DeprecatedCoreForm
