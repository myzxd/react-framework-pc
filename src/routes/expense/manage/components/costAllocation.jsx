...
class CostAllocation extends Component {
  componentDidMount() {
    ...
    const {
      cost_allocation_list: costAllocationList = [],       // 成本费用记录分摊清单
      // cost_center_type: costCenterType,           // 成本中心归属类型名称
      // cost_accounting_id: costAccountingId,         // 费用科目ID
      application_order_id: applicationOrderId,       // 归属审批单ID
  ...
  fetchAllocationName = (item) => {
    const {
      platform_name: platformName, // 平台
      supplier_name: supplierName, // 供应商
      city_name: cityName, // 城市
      biz_district_name: bizDistrictName, // 商圈
    } = item;
  ...
  renderCostShare = () => {
    ...
    const {
      cost_allocation_list: costItems = [], // 成本分摊子项目
      allocation_mode: allocationMode, // 分摊方式
    } = detail;
