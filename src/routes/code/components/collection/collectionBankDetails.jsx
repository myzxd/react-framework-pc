...
function CollectionBankDetails(props) {
  ...
  }, [collectionBankDetails, namespace]);

  useEffect(() => {
    const dataScoure = collectionBankDetails[namespace] || [];
    if (update !== true && defaultFlag.current === true &&
      (is.not.existy(dataScoure) || is.empty(dataScoure)) && onChange) {
      defaultFlag.current = false;
      onChange(undefined);
    }
  ...
  }, [collectionBankDetails, namespace, onChange, value, update]);
