...
import { Unit, CodeApproveOrderType } from '../../../application/define';
import { CoreTabs } from '../../../components/core';
import { canOperateModuleCodePayOrder } from '../../../application/define/operate';
...
const ApproveCreate = (props) => {
  ...
  const onSubmitSucessCallback = () => {
    setLoading(false);
    // 判断是否有列表权限
    if (canOperateModuleCodePayOrder()) {
      message.success('创建成功');
      window.location.href = '/#/Code/PayOrder';
      return;
    }
    message.success('创建成功');
    message.error('你没有付款审批列表权限，请联系管理进行添加');
  ...
  // 差旅编辑
  const onTravelIsShowEdit = (index) => {
    costFormItem[index].isShowEdit = !costFormItem[index].isShowEdit;
    setcostFormItem([...costFormItem]);
  };
    // 渲染关联信息
  ...
  const renderTab = () => {
    ...
    const tabItems = [
      ...
      {
        title: '主题标签',
        content: <ThemeTags {...themeTagProps} />,
        key: 'themeLabel',
      },
    ];
            ...
            costFormItem.map((v, i) => {
              return (
                <TravelBusiness
                  ...
                  onIsShowEdit={onTravelIsShowEdit}
                  onInterfaceDetail={onInterfaceDetail}
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <div ref={scrollBox} style={{ overflowY: 'scroll', flex: 1 }}>
        {/* 基本信息 */}
        ...
        {
          // 判断长度等于0时显示按钮
          costFormItem.length === 0 ? renderButttonAdd() : null
        }
      </div>
      {
        // 判断长度大于0时显示按钮
