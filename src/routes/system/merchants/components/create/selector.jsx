...
class SelectorAll extends Component {
  ...
  static defaultProps = {
    supplierInput: selector.part,         // 供应商左侧下拉框默认选择part
    cityInput: selector.part,                     // 城市左侧下拉框默认选择part
    districtInput: selector.part,         // 商圈左侧下拉框默认选择part
  }
  ...
  onChangeSelector = (v, options = []) => {
    ...
    onChange(selectorType, v, selectorAll, spell);
  }
  ...
  // 渲染供应商
  renderSupplier = () => {
    const { namespace, platforms, suppliersAll } = this.props;
    const { getFieldDecorator } = this.props.form;

    return (
      <InputGroup className={styles['app-comp-white-list-selector-input-group']}>
        <Select value={this.props.supplierInput} className={styles['app-comp-white-list-selector']} onChange={this.onSelectorAll}>
  ...
  // 渲染城市
  renderCity = () => {
    const { namespace, platforms, suppliers, suppliersAll, citiesAll } = this.props;
    const { getFieldDecorator } = this.props.form;

    return (
      <InputGroup className={styles['app-comp-white-list-selector-input-group']}>
        <Select value={this.props.cityInput} className={styles['app-comp-white-list-selector']} disabled={suppliersAll} onChange={this.onSelectorAll}>
          <Option value={selector.all}>全部</Option>
          <Option value={selector.part}>自定义</Option>
        </Select>
  ...
  // 渲染商圈
  renderDistrict = () => {
    const { namespace, platforms, suppliers, cities, citiesAll, districtsAll } = this.props;
    const { getFieldDecorator } = this.props.form;

    return (
      <InputGroup className={styles['app-comp-white-list-selector-input-group']}>
        <Select value={this.props.districtInput} className={styles['app-comp-white-list-selector']} disabled={citiesAll} onChange={this.onSelectorAll}>
          <Option value={selector.all}>全部</Option>
          <Option value={selector.part}>自定义</Option>
        </Select>
